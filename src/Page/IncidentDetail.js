...
//             onClick={handleSubmit}
//             disabled={!selected || loading}
//             className="flex-1 h-10 rounded-lg bg-[#022F72] text-white text-sm font-medium hover:bg-[#022F72]/90 disabled:opacity-50 transition-colors"
//           >
//             {loading ? 'Saving…' : 'Confirm'}
//           </button>
//         </div>
...
//           ].map(({ key, label, type }) => (
//             <label key={key} className="grid gap-2 text-sm text-text-secondary-light dark:text-text-secondary-dark">
//               {label}
//               {type === 'textarea' ? (
//                 <textarea
//                   value={form[key]}
...
//         </div>
//         <div className="mt-6 flex justify-end gap-3">
//           <button onClick={onClose} className="h-10 rounded-lg border border-border-light dark:border-border-dark px-4 text-sm font-medium text-text-light dark:text-text-dark hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors">
//             Cancel
//           </button>
//           <button onClick={handleSubmit} className="h-10 rounded-lg bg-[#022F72] text-white px-4 text-sm font-medium hover:bg-[#022F72]/90 transition-colors">
//             Save
//           </button>
//         </div>
//       </div>
...
//     setIsMenuOpen(false);
//     if (action === 'Edit Report') {
//       setEditDraft({ title: reportData?.title || '', description: reportData?.description || '', location: reportData?.location || '', category: reportData?.category || '' });
//       setShowEditModal(true);
//       return;
//     }
//     if (action === 'Share') {
//       try {
//         if (navigator.share) { await navigator.share({ title: reportData?.title || `Report #${reportId}`, text: reportData?.description || '', url: window.location.href }); showMessage('Report shared successfully.'); }
//         else if (navigator.clipboard) { await navigator.clipboard.writeText(window.location.href); showMessage('Report link copied to clipboard.'); }
//         else { window.alert('Sharing is not supported in this browser.'); }
//       } catch { showMessage('Unable to share report.'); }
//       return;
//     }
//     if (action === 'Print') { window.print(); return; }
//     if (action === 'Archive') { setReportData((prev) => (prev ? { ...prev, status: 'Archived' } : prev)); showMessage('Report archived locally.'); return; }
//     if (action === 'Delete Report') { if (window.confirm('Are you sure you want to delete this report?')) navigate('/reports'); return; }
//   }, [navigate, reportData, reportId, showMessage]);
...
//     setDetailsError('');
//     try {
//       const res = await fetch(`${BASE_URL}/GetReportDetails/${reportId}`, { headers: getAuthHeaders() });
//       const text = await res.text();
//       const data = text ? JSON.parse(text) : null;
//       if (!res.ok || !data || data?.message === 'Report not found.') {
//         setDetailsError(data?.message || `Report #${reportId} not found.`);
...
//           items?.map((item) => ({
//             status: item.status || item.statusName || 'Pending',
//             message: item.message || (item.updatedBy ? `Updated by ${item.updatedBy}` : item.citizenName ? `Updated by ${item.citizenName}` : 'Status changed'),
//             timestamp: item.time || item.timestamp || item.updatedAt,
//           })) || [];
//         const mapped = {
//           ...data,
...
//           location: data.location || data.address || 'Location not available',
//           photo: data.photo || '',
//           reporter: { name: data.citizenName || data.reporter?.name || '—', time: data.time || data.createdAt || data.reporter?.time },
//           aiAnalysis: data.aI_Analysis ? { category: data.aI_Analysis.predicted || data.aI_Analysis.category, confidence: data.aI_Analysis.score || data.aI_Analysis.confidence, entities: data.aI_Analysis.entities || [] } : null,
//           createdAt: data.time,
//         };
//         setReportData(mapped);
//         setTimeline(normalizeTimeline(data.history || data.timeline));
...
//     setTimelineError('');
//     try {
//       const res = await fetch(`${BASE_URL}/GetReportActivity?reportId=${reportId}`, { headers: getAuthHeaders() });
//       const text = await res.text();
//       const data = text ? JSON.parse(text) : null;
...
//   useEffect(() => { fetchDetails(); fetchActivity(); }, [fetchDetails, fetchActivity]);

//   const handleRefresh = () => { fetchDetails(); fetchActivity(); };
//   const handleStatusUpdated = (newStatus) => { setReportData((prev) => prev ? { ...prev, status: newStatus } : prev); fetchActivity(); };

//   if (loadingDetails) return <div className="p-8 text-center text-text-light dark:text-text-dark animate-pulse">Loading Report Details…</div>;

//   if (detailsError) return (
//     <div className="p-8 text-center">
//       <p className="text-red-500 font-medium mb-2">{detailsError}</p>
//       <p className="text-slate-500 text-sm">Report ID: {reportId}</p>
//       <button onClick={handleRefresh} className="mt-4 px-4 py-2 text-sm rounded-lg bg-[#022F72] text-white hover:bg-[#022F72]/90">Retry</button>
//     </div>
//   );
...
//   const apiRoot  = API_BASE_URL.replace(/\/api\/?$/, '');
//   const photoSrc = reportData?.photo
//     ? reportData.photo.startsWith('http') ? reportData.photo
//       : reportData.photo.startsWith('/') ? `${apiRoot}${reportData.photo}`
//       : reportData.photo
//     : null;
//     console.log("photo =", reportData?.photo);
...
//               {reportData?.title || `Report #${reportId}`}
//             </h1>
//             <p className="text-text-secondary-light dark:text-text-secondary-dark text-sm mt-1">Report ID: {reportId}</p>
//           </div>
//           <div className="flex items-center gap-2 relative">
//             <button onClick={downloadReport} className="flex items-center justify-center gap-2 h-10 px-4 text-sm font-medium rounded-lg bg-content-light dark:bg-content-dark border border-border-light dark:border-border-dark text-text-light dark:text-text-dark hover:bg-primary/10 transition-colors">
//               <span className="material-symbols-outlined text-base">download</span>
//               Download Report
//             </button>
//             <button onClick={() => setIsDetailsExpanded(!isDetailsExpanded)} className="p-2 text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-md transition-all" title="Toggle details">
//               <span className={`material-symbols-outlined transition-transform ${isDetailsExpanded ? 'rotate-180' : ''}`}>expand_more</span>
//             </button>
//             <button onClick={handleRefresh} className={`p-2 text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-md transition-all ${loadingDetails || loadingTimeline ? 'animate-spin' : ''}`} title="Refresh">
//               <span className="material-symbols-outlined">refresh</span>
//             </button>
//             <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="p-2 text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-md transition-all" title="More options">
//               <span className="material-symbols-outlined">more_vert</span>
...
//                     { label: 'Delete Report', icon: 'delete', danger: true },
//                   ].map(({ label, icon, danger }) => (
//                     <button key={label} onClick={() => handleMenuAction(label)} className={`w-full text-left px-4 py-2 hover:bg-slate-100 dark:hover:bg-slate-700 text-sm flex items-center gap-2 ${danger ? 'text-red-600 dark:text-red-400' : 'text-slate-700 dark:text-slate-300'}`}>
//                       <span className="material-symbols-outlined text-base">{icon}</span>
//                       {label}
//                     </button>
//                   ))}
...
//                 <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
//                   <div className="rounded-lg aspect-video bg-gray-800 flex items-center justify-center overflow-hidden">
//                     {photoSrc ? <img src={photoSrc} alt={reportData?.title || 'Report media'} className="w-full h-full object-cover rounded-lg" /> : <span className="text-white text-sm">No media</span>}
//                   </div>
//                 </div>
//               </div>
//               <div className="bg-content-light dark:bg-content-dark p-6 rounded-xl border border-border-light dark:border-border-dark">
//                 <h2 className="text-text-light dark:text-text-dark text-xl font-bold mb-4">Location</h2>
//                 <p className="text-text-secondary-light dark:text-text-secondary-dark text-sm">{reportData?.location || 'Location not available'}</p>
//               </div>
//             </div>
...
//               {/* ✅ Actions — Update Status فقط، لون أزرق داكن */}
//               <div className="bg-content-light dark:bg-content-dark p-6 rounded-xl border border-border-light dark:border-border-dark">
//                 <h2 className="text-text-light dark:text-text-dark text-xl font-bold mb-4">Actions</h2>
//                 <button
//                   onClick={() => setShowStatusModal(true)}
//                   className="flex items-center justify-center gap-2 h-10 px-4 text-sm font-medium rounded-lg bg-[#022F72] text-white hover:bg-[#022F72]/90 transition-colors w-full"
//                 >
//                   Update Status
//                 </button>
//               </div>

//               {/* Reporter + AI + Timeline */}
//               <div className="bg-content-light dark:bg-content-dark rounded-xl border border-border-light dark:border-border-dark divide-y divide-border-light dark:divide-border-dark">
//                 <div className="p-6">
//                   <h2 className="text-text-light dark:text-text-dark text-xl font-bold mb-4">Reporter Information</h2>
//                   <div className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-3 text-sm">
...
//                                 <span className="material-symbols-outlined text-base">{cfg.icon}</span>
//                               </div>
//                               {index !== timeline.length - 1 && <div className="w-px flex-grow bg-border-light dark:border-border-dark mt-1" />}
//                             </div>
//                             <div className="pb-1">
//                               <p className="font-medium text-text-light dark:text-text-dark text-sm">{item.message}</p>
...
//       {showStatusModal && (
//         <UpdateStatusModal currentStatus={status} reportId={reportId} onClose={() => setShowStatusModal(false)} onUpdated={handleStatusUpdated} />
//       )}
//       {showEditModal && editDraft && (
//         <EditReportModal initialValues={editDraft} onClose={() => setShowEditModal(false)} onSave={handleSaveEditedReport} />
//       )}
//     </div>
//   );
// };
...
const IncidentDetail = () => {
  ...
  // // ✅ تعديل هنا لإضافة encodeURI للتعامل مع الحروف العربية في اسم الملف
  // const photoSrc = reportData?.photo
  //   ? reportData.photo.startsWith('http') 
  //     ? encodeURI(reportData.photo)
  //     : reportData.photo.startsWith('/') 
  //       ? `${apiRoot}${encodeURI(reportData.photo)}`
  //       : encodeURI(reportData.photo)
  //   : null;
  ...
  // 🔄 معالجة الامتداد لتحويل .jpeg إلى .jpg تلقائيًا
  let rawPhoto = reportData?.photo;
  if (rawPhoto && typeof rawPhoto === 'string') {
    rawPhoto = rawPhoto.replace(/\.jpeg$/i, '.jpg');
  }
  
  // ✅ التعديل هنا للتعامل مع الحروف العربية بعد تحويل الامتداد
  const photoSrc = rawPhoto
    ? rawPhoto.startsWith('http') 
      ? encodeURI(rawPhoto)
      : rawPhoto.startsWith('/') 
        ? `${apiRoot}${encodeURI(rawPhoto)}`
        : encodeURI(rawPhoto)
    : null;

  console.log("photo =", reportData?.photo);
  console.log("converted rawPhoto =", rawPhoto);
  console.log("photoSrc =", photoSrc);
